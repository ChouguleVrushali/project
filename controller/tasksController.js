...
import {method,pathname,data,queryPara} from "../app"
let i=1,archive=[];
export class TasksController{
    controller(req,res){        
/*1>create a new task*/
    //post /tasks
    if(method==="POST" && pathname.length===2){
...
/*2>get all task*/
    //GET /tasks
    else if(method==="GET" && pathname.length===2){
        const token=checkUserAuthentication(req,res);
        if(token){
        try{
            let num=0,array=[];
...
/*3>get a specific task by id*/
    //GET /tasks/:id
   else if(method==="GET" && pathname.length===3 && num>0){
    const token=checkUserAuthentication(req,res);
    if(token){
        try{
            let k=0;
...
/*4>update the task*/
    //PUT /tasks/:id
    else if(method==="PUT" && pathname.length===3){
        const token=checkUserAuthentication(req,res);
        if(token){
            try{
                let k=0;
...
/*5>delete a task by id*/
    // DELETE /tasks/:id
    else if(method==="DELETE" && pathname.length===3 && num>0){
        const token=checkUserAuthentication(req,res);
        if(token){
            try{
                let k=0;
...
/*6>Task prioritization*/
    ...
    else if(method==="PATCH" && pathname.length===4 && pathname[3]==="priority"){
        const token=checkUserAuthentication(req,res);
        if(token){
            try{
                let k=0;
...
/*7>Assign task to user*/
    ...
    else if(method==="PATCH" && pathname.length===4 && pathname[3]==="assign"){
        const token=checkUserAuthentication(req,res);
        if(token){
            try{
                let k=0;
...
/*8>Unassign task */
   ...
   else if(method==="PATCH" && pathname.length===4 && pathname[3]==="unassign"){
    const token=checkUserAuthentication(req,res);
    if(token){
        try{
            let k=0;
...
/*9>categorize tasks*/ 
    ...
    else if(method==="PATCH" && pathname.length===4 && pathname[3]==="categorize"){
        const token=checkUserAuthentication(req,res);
        if(token){
            try{
                let k=0;
...
/*10>history*/
    ...
    else if(method==="GET" && pathname.length===4 && pathname[3]==="history"){
        const token=checkUserAuthentication(req,res);
        if(token){
            try{
                let k=0;
...
/*11>Task commenting*/
    ...
    else if(method==="POST" && pathname.length===4 && pathname[3]==="comments"){
        const token=checkUserAuthentication(req,res);
        if(token){
            try{
                let k=0;
...
/*12>search tasks*/
    //GET /tasks/search
    else if(method==="GET" && pathname.length===3 && pathname[2]==="search"){
        const token=checkUserAuthentication(req,res);
        if(token){
            try{
                let arr=[];
...
/*13>Task completion*/
    ...
    else if(method==="PATCH" && pathname.length===4 && pathname[3]==="complete"){
        const token=checkUserAuthentication(req,res);
        if(token){
            try{
                let k=0;
...
/*14>complete all pending tasks*/
    ...
    else if(method==="PATCH" && pathname.length===3 && pathname[2]==="complete-all"){
        const token=checkUserAuthentication(req,res);
        if(token){
            try{
                let arr=[];
...
/*15>Delete all completed tasks*/
    ...
    else if(method==="DELETE" && pathname.length===3 && pathname[2]==="delete-completed"){
        const token=checkUserAuthentication(req,res);
        if(token){
            try{
                let count=0,j=0;
...
/*16>due date remainder*/
    //GET /tasks/due-soon
    else if(method==="GET" && pathname.length===3 && pathname[2]==="due-soon"){
        const token=checkUserAuthentication(req,res);
        if(token){
            try{
                let arr=[];
...
/*17>overdue tasks*/
    //GET /tasks/overdue
    else if(method==="GET" && pathname.length===3 && pathname[2]==="overdue"){
        const token=checkUserAuthentication(req,res);
        if(token){
            try{
                let arr=[];
...
/*18>Task duplication*/
    ...
    else if(method==="POST" && pathname.length===4 && pathname[3]==="duplicate"){
        const token=checkUserAuthentication(req,res);
        if(token){
            try{
                let k=0,arr;
...
/*19>Archive completed tasks*/
    ...
    else if(method==="PATCH" && pathname.length===4 && pathname[3]==="archive"){
        const token=checkUserAuthentication(req,res);
        if(token){
            try{
                let k=0;
...
/*20>task sharing*/
    //POST /tasks/:id/share
    else if(method==="POST" && pathname.length===4 && pathname[3]==="share"){
        let k=0;
    }
